...
__global__ void ker_attn_softmax_lt32(T *inp, const T *attn_mask, int from_len,
  ...
  T mval[ele_per_thread]; // mask value, load from global memory
  if (attn_mask) {
    attn_mask += batch_id * to_len;
    BlockLoad(ts_load).Load(attn_mask, mval, to_len, REDUCE_FLOAT_INF_NEG);
  }

  inp += flat_3dim(batch_id, head_id, 0, nhead, from_len * to_len); // move inp pointer to the correct position
  for (int token_id = blockIdx.x * token_per_reduce; token_id < from_len;
       token_id += gridDim.x * token_per_reduce) {
    T inp_val[token_per_reduce][ele_per_thread]; // input values, load from global memory
    for (int i = 0; i < token_per_reduce && (token_id + i) < from_len; i++) {
      BlockLoad(ts_load).Load(inp + (token_id + i) * to_len, inp_val[i], to_len,
                              REDUCE_FLOAT_INF_NEG); // use BlockLoad load to_len elements into register (inp_val)
    }
    ...
    // warp reduce max, compare between 32 threads
    ...
    for (int i = 0; i < token_per_reduce; i++) {
      l_sum[i] = 0.f;
      for (int j = 0; j < ele_per_thread; j++) {
        val[i][j] = __expf(val[i][j] - l_max[i]); // avoid overflow
        l_sum[i] += val[i][j];
      }
    }
    // warp reduce sum, sum all 32 threads
...
template <typename T, int block_dim, int ele_per_thread> // elements number for each thread
__global__ void ker_attn_softmax(T *inp, const T *attn_mask, int from_len,
                                 int to_len, bool mask_future) {
  /*
  if mask_future is true, then if column_number > row_number,
  ...
  for (int token_id = blockIdx.x * token_per_reduce; token_id < from_len;
    ...
    /* step 1. each thread compute its max */
    float val[token_per_reduce][ele_per_thread];
    float l_max[token_per_reduce]; // thread local max: each thread computes the local maximum of ele_per_thread elements
    // BEGIN ASSIGN4_1_1
    for (int i = 0; i < token_per_reduce; i++) {
      l_max[i] = REDUCE_FLOAT_INF_NEG; // initialize to -inf
      for (int j = 0; j < ele_per_thread; j++) {
        float temp_val;
        if (mask_future && (ele_per_thread * threadIdx.x + j) > (token_id + i)) { // if column_number > row_number, set the value to -inf.
          temp_val = REDUCE_FLOAT_INF_NEG;
        } else {
          temp_val = (float)inp_val[i][j]; // initialize temp_val with input value
          if (attn_mask) {
            temp_val += (float)mval[j];
          }
        }
        val[i][j] = temp_val;
        l_max[i] = fmaxf(l_max[i], temp_val); // compare two floats, return max one
      }
    }
    // END ASSIGN4_1_1
    // block reduce max, record largest number among all warps
    ...
    for (int i = 0; i < token_per_reduce; i++) {
      l_sum[i] = 0.f;
      for (int j = 0; j < ele_per_thread; j++) {
        val[i][j] = __expf(val[i][j] - s_max[i]); // avoid overflow
        l_sum[i] += val[i][j];
      }
    }
    // END ASSIGN4_1_1
    // block reduce sum, record sum among all warps
    blockReduce<ReduceType::kSum, token_per_reduce>(l_sum);
    // write shared for token_per_reduce lines
...
__global__ void ker_attn_softmax_bw(T *grad, const T *inp, int softmax_length) {
  /*
  calculate backward of softmax function.
  */
...
void launch_attn_softmax_bw(float *out_grad,
  ...
  int size = rows * softmax_len * sizeof(float); // rows = batch * nhead * from_len
  // float *d_grad, *d_inp;
  // cudaMalloc((void **)&d_inp, size);
  // cudaMalloc((void **)&d_grad, size);

  // allocate a single chunk of memory for both d_inp and d_grad to improve memory locality
  float *d_mem;
  cudaMalloc((void **)&d_mem, size * 2);

  float *d_inp = d_mem;
  float *d_grad = d_mem + rows * softmax_len;

  cudaMemcpy(d_inp, soft_outp, size, cudaMemcpyHostToDevice);
  cudaMemcpy(d_grad, out_grad, size, cudaMemcpyHostToDevice);
  ...
  if (softmax_len <= 32) {
    ...
    ker_attn_softmax_bw<float, 8><<<grid_dim, block_dim, 0, stream>>>(
      d_grad, d_inp, softmax_len);
  } else if (softmax_len <= 384) { // 384/32=12
  ...
  // Copy back to the host
  cudaMemcpy(out_grad, d_grad, size, cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
