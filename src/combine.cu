    ...
    const int *b_strides)
{
  ...
  // A: [M, N], B: [N, P], out: [M, P]
  int M = a_shape[1]; // [batch, row, col]
  int N = a_shape[2];
  int P = b_shape[2];

  int tx = threadIdx.x; // x correspond to column, means current thread's rank
  int ty = threadIdx.y; // y correspond to row 

  // int row = blockIdx.y * blockDim.y + ty; // row in out
  // int col = blockIdx.x * blockDim.x + tx; // col in out
  ...
  for (int k = 0; k < N; k += TILE) {
    // move tile A, A[row][k + tx]
    if (row < M && (k+tx) < N) {
      a_shared[ty][tx] = a_ptr[row * a_strides[1] + (tx + k) * a_strides[2]];
    } else {
      a_shared[ty][tx] = 0.0f;
    }

    // move tile B, B[k + ty][col]
    if ((k+ty) < N && col < P) {
      b_shared[ty][tx] = b_ptr[(ty + k) * b_strides[1] + col * b_strides[2]];
    } else {
      b_shared[ty][tx] = 0.0f;
    }
    __syncthreads();
    ...
    for (int i = 0; i < TILE; i++) {
      acc += a_shared[ty][i] * b_shared[i][tx];
    }

    __syncthreads();
  }

  // write to global memory in gpu
  if (row < M && col < P) {
    out_ptr[row * out_strides[1] + col * out_strides[2]] = acc;
  }
  /// END HW1_4
}
    ...
    int fn_id)
{
  ...
  // global position this thread is working on
  int gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid > out_size) return;

  // need to compute out[out_pos] = func(in_storage[in_pos])
  // 1. get out_index from gid
  to_index(gid, out_shape, out_index, shape_size); // __device__ void to_index(int ordinal, const int *shape, int *out_index, int num_dims)

  // 2. get in_index from broadcast
  broadcast_index(out_index, out_shape, in_shape, in_index, shape_size, shape_size); // __device__ void broadcast_index(const int *big_index, const int *big_shape, const int *shape, int *out_index, int num_dims_big, int num_dims)

  // 3. get position from index in memory
  int out_pos = index_to_position(out_index, out_strides, shape_size); // __device__ int index_to_position(const int *index, const int *strides, int num_dims)
    ...
    int fn_id)
{
  ...
  // Each block computes ONE output element (one reduction).
  int gid = (int)(blockIdx.x * blockDim.x + threadIdx.x);
  if (gid >= out_size) return;

  // get multi-dim index of output
  ...
  // write result using out strides for proper position
  int out_store_pos = index_to_position(out_index, out_strides, shape_size);
  out[out_store_pos] = acc;
  /// END HW1_3
}
    ...
    int fn_id)
{
  ...
  // need to compute out[out_pos] = func(in_storage[in_pos])
  // 1. get out_index from gid
  to_index(gid, out_shape, out_index, out_shape_size); // __device__ void to_index(int ordinal, const int *shape, int *out_index, int num_dims)

  // 2. get in_index from broadcast for two input
  broadcast_index(out_index, out_shape, a_shape, a_index, out_shape_size, a_shape_size); // __device__ void broadcast_index(const int *big_index, const int *big_shape, const int *shape, int *out_index, int num_dims_big, int num_dims)
  broadcast_index(out_index, out_shape, b_shape, b_index, out_shape_size, b_shape_size);

  // 3. get position from index in memory
  int out_pos = index_to_position(out_index, out_strides, out_shape_size); // __device__ int index_to_position(const int *index, const int *strides, int num_dims)
  int in_pos_a = index_to_position(a_index, a_strides, out_shape_size);
  int in_pos_b = index_to_position(b_index, b_strides, out_shape_size);

  // 4. apply functions
  out[out_pos] = fn(fn_id, a_storage[in_pos_a], b_storage[in_pos_b]);
