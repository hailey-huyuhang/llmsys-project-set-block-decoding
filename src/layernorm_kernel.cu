...
__global__ void ker_layer_norm(T *ln_res, T *vars, T *means, const T *inp,
  ...
  // Step 1
  // hidden_dim >>= 2 in launch function, so this hidden_size is original/4, we can directly cast to float4 for speedup
  float l_sum = 0; // local sum for every thread
  float l_square_sum = 0; // local square sum for every thread
  // origin: [token0: h0 h1 h2 ... h767 | token1: h0 h1 ... h767 | ...]
  // float4 verision: [token0: f4_0 f4_1 ... f4_191 | token1: ...]
  const float4 *inp_f4 = reinterpret_cast<const float4 *>(inp) + blockIdx.x * hidden_size; // cast to float4
  // interleaved assignment, each thread deal with 1-2 float4
  for (uint idx = threadIdx.x; idx < hidden_size; idx += blockDim.x) {
    float4 val = inp_f4[idx]; // read 4 float at once
    l_sum += val.x + val.y + val.z + val.w; // add 4 elements into local sum
    l_square_sum += val.x * val.x + val.y * val.y + val.z * val.z + val.w * val.w;
  }

  // Step 2
  float vals[2] = {l_sum, l_square_sum};
  blockReduce<ReduceType::kSum, 2>(vals);

  __shared__ float s_mean, s_var;
  if (threadIdx.x == 0) {
    int total = hidden_size * 4; // revert to original size
    s_mean = vals[0] / total;
    float variance = vals[1] / total - s_mean * s_mean + LN_EPSILON;
    s_var = rsqrt(variance); // rsqrtf(x) = 1/sqrt(x)
    if (means != nullptr) {
  ...
  // Step 3
  // calculate x_hat and *scale+bias individually
  ...
  for (uint idx = threadIdx.x; idx < hidden_size; idx += blockDim.x) {
    ...
    res.x = (s_var * (val.x - s_mean)) * s.x + b.x;
    res.y = (s_var * (val.y - s_mean)) * s.y + b.y;
    res.z = (s_var * (val.z - s_mean)) * s.z + b.z;
    res.w = (s_var * (val.w - s_mean)) * s.w + b.w;
    out_f4[idx] = res;
  }
  /// END ASSIGN4_2_1
}
...
__global__ void ker_ln_bw_dgamma_dbetta(T *gamma_grad, T *betta_grad,
  ...
  // Step 1, for all rows, add local gradient
  ...
  for (int row = threadIdx.y; row < rows; row += TILE_DIM) {
    if (col < width) {
      float dy = out_grad[row * width + col];
      float x = inp[row * width + col];
      float xhat = (x - means[row]) * rsqrt(vars[row]);
      local_dbetta += dy; // bias
      local_dgamma += dy * xhat; // scale
    }
  }
  ...
  // Step 3, each thread read shared memory, use shfl_down to reduce
  float betta_val = betta_buffer[threadIdx.y][threadIdx.x];
  float gamma_val = gamma_buffer[threadIdx.y][threadIdx.x]; // transpose

  for (int offset = TILE_DIM / 2; offset > 0; offset /= 2) {
    betta_val += g.shfl_down(betta_val, offset);
    gamma_val += g.shfl_down(gamma_val, offset);
  }
  
  // Step 4, write to global memory
  if (threadIdx.x == 0) {
    int out_col = blockIdx.x * TILE_DIM + threadIdx.y;
    if (out_col < width) {
      betta_grad[out_col] = betta_val;
      gamma_grad[out_col] = gamma_val;
    }
  }
  /// END ASSIGN4_2_2
}
...
__global__ void ker_ln_bw_dinp(T *inp_grad, const T *out_grad, const T *inp,
  ...
  float rsqrt_val = rsqrt(var_val); // var_val = var^2 + LN_EPSILON
  int total = hidden_dim * 4;

  // hidden_dim already / 4
  ...
  for (uint idx = threadIdx.x; idx < hidden_dim; idx += blockDim.x) {
    ...
    float xh_x = (x.x - mean_val) * rsqrt_val; // calculate x_hat again to save memory
    float xh_y = (x.y - mean_val) * rsqrt_val;
    float xh_z = (x.z - mean_val) * rsqrt_val;
    float xh_w = (x.w - mean_val) * rsqrt_val;

    sum_dxhat += dx_x + dx_y + dx_z + dx_w; // first sum in bracket
    sum_dxhat_xhat += dx_x * xh_x + dx_y * xh_y + dx_z * xh_z + dx_w * xh_w; // second sum in bracket
  }
   
  // Step 3, reduce
  float vals[2] = {sum_dxhat, sum_dxhat_xhat};
  blockReduce<ReduceType::kSum, 2>(vals);

  __shared__ float s_sum_dxhat, s_sum_dxhat_xhat;
  if (threadIdx.x == 0) {
    s_sum_dxhat = vals[0] / total;
    s_sum_dxhat_xhat = vals[1] / total;
  }
  __syncthreads();
 
  // Step 4, calculate final gradient
  float c1 = rsqrt_val;
  float c2 = -rsqrt_val * rsqrt_val * s_sum_dxhat_xhat;
  float c3 = -c2 * mean_val - rsqrt_val * s_sum_dxhat;

  float4 *inp_grad_f4 = reinterpret_cast<float4 *>(inp_grad) + blockIdx.x * hidden_dim;

  for (uint idx = threadIdx.x; idx < hidden_dim; idx += blockDim.x) {
    float4 dy = out_grad_f4[idx];
    float4 gm = gamma_f4[idx];
    float4 x = inp_f4[idx];
    float4 res;
    ...
    // dxhat = dy.x * gm.x;
    // xhat  = (x.x - mean_val) * rsqrt_val;
    // res.x = rsqrt_val * (dxhat - s_sum_dxhat - xhat * s_sum_dxhat_xhat);

    // dxhat = dy.y * gm.y;
    // xhat  = (x.y - mean_val) * rsqrt_val;
    // res.y = rsqrt_val * (dxhat - s_sum_dxhat - xhat * s_sum_dxhat_xhat);

    // dxhat = dy.z * gm.z;
    // xhat  = (x.z - mean_val) * rsqrt_val;
    // res.z = rsqrt_val * (dxhat - s_sum_dxhat - xhat * s_sum_dxhat_xhat);

    // dxhat = dy.w * gm.w;
    // xhat  = (x.w - mean_val) * rsqrt_val;
    // res.w = rsqrt_val * (dxhat - s_sum_dxhat - xhat * s_sum_dxhat_xhat);

    res.x = c1 * (dy.x * gm.x) + c2 * x.x + c3;
    res.y = c1 * (dy.y * gm.y) + c2 * x.y + c3;
    res.z = c1 * (dy.z * gm.z) + c2 * x.z + c3;
    res.w = c1 * (dy.w * gm.w) + c2 * x.w + c3;
