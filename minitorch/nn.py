...
def logsumexp(input: Tensor, dim: int) -> Tensor:
    ...
    """  
    # COPY FROM ASSIGN2_2
    max_val = max(input, dim) # def max(input: Tensor, dim: int) -> Tensor:
...
def softmax_loss(logits: Tensor, target: Tensor) -> Tensor:
    ...
    target_mask = one_hot(target, C)
    
    # filter the target position, and squeez dimension
    target_logits = (logits * target_mask).sum(dim=1)

    result  = lse - target_logits # loss
