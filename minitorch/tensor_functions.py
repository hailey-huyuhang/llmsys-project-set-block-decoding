...
class Attn_Softmax(Function):
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tensor:
      ...
      grad = out_grad.backend.attn_softmax_bw(out_grad, soft_inp)
      # forward has input and mask as parameter, backward return two gradient, but mask has no gradient
...
class LayerNorm(Function):
    ...
    def forward(ctx: Context, inp: Tensor, gamma: Tensor, beta: Tensor) -> Tensor:
      #   BEGIN ASSIGN4_2_1
      ln_res_flat, vars, means = inp.backend.layernorm_fw(inp, gamma, beta)
      out = tensor_from_numpy(
            ln_res_flat.reshape(inp.shape),
            backend=inp.backend
      )
      ctx.save_for_backward(
            inp, gamma, beta,
            tensor_from_numpy(vars, backend=inp.backend),
            tensor_from_numpy(means, backend=inp.backend)
      )
      return out
      #   END ASSIGN4_2_1
    ...
    #         tensor_from_numpy(gamma_grad, backend=inp.backend),
    #         tensor_from_numpy(beta_grad, backend=inp.backend),
    #   )
      #   END ASSIGN4_2_2
