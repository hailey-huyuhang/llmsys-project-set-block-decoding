...
class CudaKernelOps(TensorOps):
    ...
    def attn_softmax_fw(inp: Tensor, mask: Tensor):
      ...
      is_dec_self_attn = False # should be decided based on the context
    ...
    def attn_softmax_bw(out_grad: Tensor, soft_inp: Tensor):
      ...
      d_grad = torch.tensor(out_grad._tensor._storage, dtype=torch.float32).cuda()
      d_inp  = torch.tensor(soft_inp._tensor._storage, dtype=torch.float32).cuda()
      ...
      lib_softmax.launch_attn_softmax_bw(
          ctypes.c_void_p(d_grad.data_ptr()),
          ctypes.c_void_p(d_inp.data_ptr()),
          rows,
          to_len,
          stream
      )

      out_grad._tensor._storage[:] = d_grad.cpu().numpy()
      return out_grad
      #   END ASSIGN4_1_2
    ...
    def layernorm_fw(inp: Tensor, gamma: Tensor, beta: Tensor):
      ...
      # assume 2D input, inp: [batch_size * seq_len, hidden_size]
      batch_size = inp.shape[0]  # batch_size * seq_len
      hidden_dim = inp.shape[1]

      ln_res = np.zeros(inp.shape, dtype=datatype)
    ...
    def layernorm_bw(out_grad: Tensor, inp: Tensor, gamma: Tensor, beta: Tensor, var: Tensor, mean: Tensor):
      ...
      from .tensor_functions import zeros_tensor_from_numpy
      
      gamma_grad = zeros_tensor_from_numpy((hidden_dim,), backend=inp.backend)
      beta_grad  = zeros_tensor_from_numpy((hidden_dim,), backend=inp.backend)
      inp_grad   = zeros_tensor_from_numpy(inp.shape, backend=inp.backend)
    ...
    #   gamma_grad = np.zeros(hidden_dim, dtype=datatype)
    #   beta_grad = np.zeros(hidden_dim, dtype=datatype)
    #   inp_grad   = np.zeros(inp.shape, dtype=datatype).flatten()
